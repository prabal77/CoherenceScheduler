...
public class CoherenceJobStore {
   ...
   public void updateStatusForFailure(TriggerListHolder triggerListHolder) {
      System.out.println("Updaing status failure. size " + triggerListHolder.size());
      if (!triggerListHolder.isEmpty()) {
         this.triggerStoreCache.invokeAll(triggerListHolder.getTriggerKeySet(), new UpdateTriggerStatusProcessor(null, ExecutionStatus.REJECTED));
      }
   }
