...
public class SchedulerServiceBuilder implements Builder<SchedulerServiceConfig> {
   ...
   public SchedulerServiceConfig realize() {
      ...
      if (this.idlewaittimeinmillis <= Long.parseLong(SchedulerContantsEnum.MISFIRE_THRESHOLD.getConstantValue())) {
         throw new ConfigurationException("Idle Time wait is less than MisFire Threshold", "Idle wait time shoud be more than " + SchedulerContantsEnum.MISFIRE_THRESHOLD.getConstantValue()
               + " miliseconds");
      }
